...
IF /I "%SIGN%" == "True" (
  SET FILES="%~dp0_bin\Filter_x86%SUFFIX%\MpcScriptSource.ax" "%~dp0_bin\Filter_x64%SUFFIX%\MpcScriptSource64.ax"
...
IF /I "%VERRELEASE%" == "1" (
  ...
  IF /I "%REVBRANCH%" == "master" (
    ...
    SET "PCKG_NAME=MpcScriptSource-%VERMAJOR%.%VERMINOR%.%VERBUILD%.%REVNUM%.%REVBRANCH%_git%REVDATE%-%REVHASH%%SUFFIX%"
  )
)
...
.\_bin\Filter_x86%SUFFIX%\MpcScriptSource.ax ^
.\_bin\Filter_x64%SUFFIX%\MpcScriptSource64.ax ^
...
TITLE Compiling MPC Script Source [FINISHED]
TIMEOUT /T 3
ENDLOCAL
EXIT
...
:SubCompiling
TITLE Compiling MPC Script Source - %BUILDCFG%^|%1...
